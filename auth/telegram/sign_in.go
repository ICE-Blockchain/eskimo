...
func (c *client) SignIn(ctx context.Context, tmaToken string) (tokens *auth.Tokens, err error) {
	if vErr := c.verifyTelegramTMAToken(tmaToken); vErr != nil {
	...
	signIn, err := c.upsertTelegramSignIn(ctx, now, userID, telegramUserID)
	if err != nil {
...
func (c *client) verifyTelegramTMAToken(tmaToken string) error {
	var vErr error
	for _, botToken := range c.cfg.TelegramBots {
...
func (c *client) upsertTelegramSignIn(ctx context.Context, now *time.Time, userID, telegramUserID string) (*telegramSignIn, error) {
	params := []any{now.Time, telegramUserID, userID}
	sql := `INSERT INTO telegram_sign_ins (
							created_at,token_issued_at, telegram_user_id, user_id,issued_token_seq,previously_issued_token_seq)
						VALUES ($1, $1, $2, $3, 1, 0)
